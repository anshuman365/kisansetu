# app/main_fixed_v2.py - COMPLETE WORKING FIX
...
class UserCreate(BaseModel):
    ...
    password: str = Field(..., min_length=6, max_length=20)  # Limited to 20 chars
...
class OrderCreate(BaseModel):
    ...
    def validate_quantity_unit(cls, v):
        valid_units = ["quintal", "ton"]
        if v not in valid_units:
            raise ValueError(f'Quantity unit must be one of: {", ".join(valid_units)}')
        return v
...
# ========== AUTH UTILITIES ==========
# Use a different algorithm to avoid bcrypt issues
